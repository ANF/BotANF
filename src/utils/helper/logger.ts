    ...
    public log(textToLog: string, logType: logLevel, file: string) {
        ...
        switch (logType) {
            case logLevel.verbose:
                if (this.minimumLoggingLevel == logLevel.verbose) color = this.colors.color_verbose;
                else return;
                break;

            case logLevel.info:
                if (this.minimumLoggingLevel == logLevel.info || this.minimumLoggingLevel < logLevel.info) color = this.colors.color_info;
                else return;
                break;

            case logLevel.warning:
                if (this.minimumLoggingLevel == logLevel.warning || this.minimumLoggingLevel < logLevel.warning) color = this.colors.color_warning;
                else return;
                break;

            case logLevel.error:
                if (this.minimumLoggingLevel == logLevel.error || this.minimumLoggingLevel < logLevel.error) color = this.colors.color_error;
                else return;
                break;
        }
        console.log(`${color}[${logLevel[logType].toUpperCase()} ${file}]${color_reset} ${textToLog}`);
        if (this.closeAppOnError == true && logType == logLevel.error) process.exit(1);
    }
}
 ...
 * The logger instance.
 */
const logger = new Logger({ minimumLoggingLevel: logLevel.verbose, closeAppOnError: true });
