            ...
            args: [
                {
                    ...
                    nullable: true,
                    description:
                        "The name of command or category by putting `c-` before the name",
                },
            ],
            description:
                "Gives information about a specified command or the list of all commands with their categories",
            usage: "`-help [category or command]`",
        });
    }

    private isUserAllowedToSeeCommand(
        userPermissions: PermissionString[],
        command: Command
    ): boolean {
        const commandPermissions: PermissionString[] =
            command.options.userPermissions || [];
        return commandPermissions.every((value) =>
            userPermissions.includes(value)
        );
    }

    private getCommandsFromCategory(
        message: Message,
        categoryName: string
    ): Command[] {
        ...
        category?.forEach((cmd) => {
            if (this.client?.isOwner(message.author.id)) {
                ...
                this.isUserAllowedToSeeCommand(
    ...
    public exec(message: Message, { name }: { name: string }) {
        ...
        if (name) {
            ...
            if (name.startsWith("c-")) {
                ...
                embed.addField(
                    "Usage:",
                    cmd?.options?.usage || "No usage example provided"
                );
                embed.addField(
                    "Parameters:",
                    cmd?.options?.args
                        ?.map(
                            (arg) =>
                                `**${this.formatWord(arg.id)}:** ${
                                    arg.nullable ? "*(Optional)* " : ""
                                }${
            ...
            embed.setDescription(
                `BotANF is discord bot that helps in some stuff like moderation, polls etc. Here's the list of all categories followed with their commands, use \`-help c-<category name>\` to get the list of commands for a category or use \`-help <command name>\` to get the details of a specific command`
            );
